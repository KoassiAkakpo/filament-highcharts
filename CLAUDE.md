...
- The Alpine `highcharts()` component calls `Highcharts.chart()` on init and listens for the Livewire `updateOptions` event (dispatched by filter changes, polling, or `updateOptions()` on the widget), deep-merging new options with `lodash.merge` before `chart.update()`. `extraJsOptions` is merged client-side on top of the PHP options.
