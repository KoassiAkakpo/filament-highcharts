...
- `vendor/bin/phpstan analyse --memory-limit=1G` — static analysis (Larastan, level 5, config in `phpstan.neon.dist`; also run in CI). `testbench.yaml` registers the package provider in Larastan's bootstrapped app so namespaced `view()` calls resolve.
- `vendor/bin/pest` — run tests (Pest + Orchestra Testbench). `tests/Feature/` covers widget behavior via `Livewire::test` (chart id stability, filter dispatch, options-hash dedup, defer loading), the generator command for every chart type in the config (plus published-stub precedence), and asset registration. Fixture widgets and a test panel live in `tests/Fixtures/`. A GitHub Action (`tests.yml`) runs the suite on PHP 8.3–8.5.
  - Gotcha encoded in `tests/TestCase.php`: Filament's `SupportServiceProvider` must be registered **before** `LivewireServiceProvider` — it `bind()`s Livewire's `DataStore` to a Filament override, and `bind()` drops any already-registered shared instance; in the wrong order every `store()` call gets a fresh empty store and Livewire component state (error bags, dispatches) silently breaks.
